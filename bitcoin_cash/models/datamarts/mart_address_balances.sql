{{ config(materialized="table", schema="datamart", cluster_by=["address"]) }}
...
with
    ...
    received as (
        ...
        select addr as address, sum(output.value) as total_received_satoshis
    ...
    -- Total BCH spent per address: sum of all input values originating from the
    -- address.
    -- Coinbase inputs are excluded as they have no real sender address.
    spent as (

        select addr as address, sum(input.value) as total_spent_satoshis
...
select
    ...
    (r.total_received_satoshis - coalesce(s.total_spent_satoshis, 0))
    / 1e8 as balance_bch

from received r
left join spent s on r.address = s.address
